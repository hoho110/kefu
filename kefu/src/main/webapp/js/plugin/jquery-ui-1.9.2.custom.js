	...
	$.widget("ui.draggable", $.ui.mouse,
			{
				...
				_createHelper : function(event) {
					...
					var o = this.options;
					var helper = $.isFunction(o.helper) ? $(o.helper.apply(this.element[0], [ event ])) : (o.helper == 'clone' ? this.element.clone()
				...
				_getParentOffset : function() {
					...
					if ((this.offsetParent[0] == document.body) // This needs to
							// be actually
							// done for all
							// browsers,
							// since
				...
				_generatePosition : function(event) {
					...
					if (this.originalPosition) { // If we are not dragging
						// yet, we won't check for
						// options
						var containment;
						if (this.containment) {
					...
					$.ui.mouse,
					{
							...
							this._renderAxis = function(target) {
								...
								for ( var i in this.handles) {
									...
									if (this.elementIsWrapper && this.originalElement[0].nodeName.match(/textarea|input|select|button/i)) {
										...
										var padPos = [ 'padding', /ne|nw|n/.test(i) ? 'Top' : /se|sw|s/.test(i) ? 'Bottom' : /^e$/.test(i) ? 'Right' : 'Left' ]
												.join("");
						...
						_mouseStop : function(event) {
							...
							if (this._helper) {
								var pr = this._proportionallyResizeElements, ista = pr.length && (/textarea/i).test(pr[0].nodeName), soffseth = ista
										 ...
										 * jump
										 * height
										 */? 0 : that.sizeDiff.height, soffsetw = ista ? 0 : that.sizeDiff.width;
						...
						_respectSize : function(data, event) {
							...
							var el = this.helper, o = this._vBoundaries, pRatio = this._aspectRatio || event.shiftKey, a = this.axis, ismaxw = isNumber(data.width)
									&& o.maxWidth && (o.maxWidth < data.width), ismaxh = isNumber(data.height) && o.maxHeight && (o.maxHeight < data.height), isminw = isNumber(data.width)
							...
							for ( var i = 0; i < this._proportionallyResizeElements.length; i++) {
								...
								if (!this.borderDif) {
									var b = [ prel.css('borderTopWidth'), prel.css('borderRightWidth'), prel.css('borderBottomWidth'),
											prel.css('borderLeftWidth') ], p = [ prel.css('paddingTop'), prel.css('paddingRight'), prel.css('paddingBottom'),
					...
					"containment",
					{
						...
						start : function(event, ui) {
							...
							// i'm a node, so compute top, left, right, bottom
								...
								var co = that.containerOffset, ch = that.containerSize.height, cw = that.containerSize.width, width = ($.ui.hasScroll(ce,
						...
						stop : function(event, ui) {
							...
							var helper = $(that.helper), ho = helper.offset(), w = helper.outerWidth() - that.sizeDiff.width, h = helper.outerHeight()
						...
						_mouseDrag : function(event) {
							...
							for ( var i = this.items.length - 1; i >= 0; i--) {
								...
								if (itemElement != this.currentItem[0] // cannot
										...
										// done
										// before
										&& !$.contains(this.placeholder[0], itemElement) // no
										// action
										// if
										// the
										// item
			...
			.widget(
					"ui.accordion",
					{
						...
						_create : function() {
							...
							this.active = this._findActive(options.active).addClass("ui-accordion-header-active ui-state-active").toggleClass(
		...
		(function($, prototype) {
			...
			prototype._create = function() {
				if (this.options.navigation) {
					var that = this, headers = this.element.find(this.options.header), content = headers.next(), current = headers.add(content).find("a")
		...
		_create : function() {
			...
			this._on(this.menu.element, {
				...
				menufocus : function(event, ui) {
					// #7024 - Prevent accidental activation of menu items in
					// Firefox
					if (this.isNewMenu) {
					...
					// back compat for _renderItem using item.autocomplete, via
					// #7810
			...
			.widget(
					"ui.button",
					{
						...
						_destroy : function() {
							...
							this.buttonElement.removeClass(baseClasses + " " + stateClasses + " " + typeClasses).removeAttr("role").removeAttr("aria-pressed")
	...
	$.widget("ui.buttonset", {
		...
		refresh : function() {
			...
			this.buttons = this.element.find(this.options.items).filter(":ui-button").button("refresh").end().not(":ui-button").button().end().map(function() {
				return $(this).button("widget")[0];
			}).removeClass("ui-corner-all ui-corner-left ui-corner-right").filter(":first").addClass(rtl ? "ui-corner-right" : "ui-corner-left").end().filter(
					":last").addClass(rtl ? "ui-corner-left" : "ui-corner-right").end().end();
		},
					...
					Datepicker.prototype,
					{
						...
						_optionDatepicker : function(target, name, value) {
							...
							if (arguments.length == 2 && typeof name == 'string') {
								return (name == 'defaults' ? $.extend({}, $.datepicker._defaults) : (inst ? (name == 'all' ? $.extend({}, inst.settings) : this
										._get(inst, name)) : null));
							}
							var settings = name || {};
							...
							for ( var row = 0; row < numMonths[0]; row++) {
								...
								for ( var col = 0; col < numMonths[1]; col++) {
									...
									for ( var dRow = 0; dRow < numRows; dRow++) { // create
										...
										// rows
										calender += '<tr>';
										var tbody = (!showWeek ? '' : '<td class="ui-datepicker-week-col">' + this._get(inst, 'calculateWeek')(printDate)
												+ '</td>');
...
(function($, undefined) {
	...
	$.widget("ui.dialog", {
		...
		_create : function() {
			...
			uiDialogTitlebar = (this.uiDialogTitlebar = $("<div>")).addClass("ui-dialog-titlebar  ui-widget-header  " + "ui-corner-all  ui-helper-clearfix")
		...
		create : function(dialog) {
			...
			$(document).bind(
					...
					function(event) {
						...
						if (instances.length !== 0 && instances[instances.length - 1] === $el && dialog.options.closeOnEscape && !event.isDefaultPrevented()
...
(function($, undefined) {
	...
	$.widget("ui.menu", {
		...
		_destroy : function() {
			// Destroy (sub)menus
			this.element.removeAttr("aria-activedescendant").find(".ui-menu").andSelf().removeClass(
					"ui-menu ui-widget ui-widget-content ui-corner-all ui-menu-icons").removeAttr("role").removeAttr("tabIndex").removeAttr("aria-labelledby")
					.removeAttr("aria-expanded").removeAttr("aria-hidden").removeAttr("aria-disabled").removeUniqueId().show();
...
(function($, undefined) {
	...
	$.widget("ui.progressbar", {
		...
		_destroy : function() {
			this.element.removeClass("ui-progressbar ui-widget ui-widget-content ui-corner-all").removeAttr("role").removeAttr("aria-valuemin").removeAttr(
	...
	$
			.widget(
					...
					$.ui.mouse,
					{
						...
						_create : function() {
							var i, handleCount, o = this.options, existingHandles = this.element.find(".ui-slider-handle").addClass(
						...
						_destroy : function() {
							...
							this.element.removeClass("ui-slider" + " ui-slider-horizontal" + " ui-slider-vertical" + " ui-slider-disabled" + " ui-widget"
			...
			.widget(
					"ui.tabs",
					{
						...
						_processTabs : function() {
							...
							this.tablist = this._getList().addClass("ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all").attr(
									"role", "tablist");
	...
	if ($.uiBackCompat !== false) {
				...
				.widget("ui.tabs", $.ui.tabs,
						{
							...
							add : function(url, label, index) {
								...
								var doInsertAfter, panel, options = this.options, li = $(options.tabTemplate.replace(/#\{href\}/g, url).replace(/#\{label\}/g,
										label)), id = !url.indexOf("#") ? url.replace("#", "") : this._tabId(li);
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			_tabId : function(tab) {
				...
				a = a[0];
				return $(a).closest("li").attr("aria-controls") || a.title && a.title.replace(/\s/g, "_").replace(/[^\w\u00c0-\uFFFF\-]/g, "")
	...
	(function() {
		...
		$.effects.animateClass = function(value, duration, easing, callback) {
			...
			return this.queue(function() {
				var animated = $(this), baseClass = animated.attr("class") || "", applyClassChange, allAnimations = o.children ? animated.find("*").andSelf()
						: animated;
...
(function($, undefined) {
	...
	$.effects.effect.fold = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects.setMode(el, o.mode || "hide"), show = mode === "show", hide = mode === "hide", size = o.size || 15, percent = /([0-9]+)%/
				.exec(size), horizFirst = !!o.horizFirst, widthFirst = show !== horizFirst, ref = widthFirst ? [ "width", "height" ] : [ "height", "width" ], duration = o.duration / 2, wrapper, distance, animation1 = {}, animation2 = {};
...
(function($, undefined) {
	...
	$.effects.effect.size = function(o, done) {
		...
		// Copy for children
		props2 = [ "width", "height", "overflow" ], cProps = [ "fontSize" ], vProps = [ "borderTopWidth", "borderBottomWidth", "paddingTop", "paddingBottom" ], hProps = [
